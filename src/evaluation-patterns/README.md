...
The eight eval patterns in this demo are things you write _around_ your agent. But the most successful coding agent harnesses have taken a different approach: they embed evaluation _inside_ the agent loop itself, making the agent its own first reviewer before any human sees the output.

**Aider's edit-lint-test-fix loop** is the most transparent example. After every LLM edit, Aider [automatically runs linters and tests](https://aider.chat/docs/usage/lint-test.html) on the modified files. If the linter or test command returns a non-zero exit code, Aider feeds the errors back to the LLM and loops — edit, lint, test, fix, repeat. This is continuous trajectory evaluation baked into the agent cycle. Aider uses tree-sitter to parse the AST and show lint errors in the context of their containing function, giving the LLM richer signal than raw error output. The tradeoff is real: if the LLM cannot fix a lint error, the loop can [run indefinitely and burn tokens](https://github.com/paul-gauthier/aider/issues/1090), which is why Aider exposes `--no-auto-lint` and `--no-auto-test` escape hatches.

**Cursor's shadow workspace** takes evaluation a step further by running it _before the user ever sees the edit_. When AI proposes a change, Cursor [applies it in a hidden Electron window](https://cursor.com/blog/shadow-workspace) — a full clone of the workspace with its own Language Server Protocol instance. The LSP analyzes the modified code and reports diagnostics (type errors, missing imports, style violations) back to the AI, which iterates until the code is clean. As Cursor's Arvid Lunnemark puts it: "Lints allow going from 90% working code to 100% working code." This is the mocked-tools principle applied to file editing — isolate the environment, evaluate the change, and only surface it to the user when it passes. Multiple AIs can interleave requests against separate shadow windows without interfering with each other.

**Amazon Q Developer** embeds evaluation most deeply through a [three-agent evaluation subsystem](https://aws.amazon.com/blogs/devops/reinventing-the-amazon-q-developer-agent-for-software-development/): a Memory agent that analyzes iteration results and maintains cross-iteration context, a Critic agent that evaluates progress and detects dead ends, and a Debugger agent that modifies the plan based on the critique. When the Critic detects a dead end — the agent equivalent of a failing eval — it triggers intelligent backtracking, rolling the agent back to a previous state rather than continuing down an unproductive path. On top of this, Q Developer generates multiple candidate solutions and selects the most promising one — essentially pass^k (Pattern 8) applied at generation time rather than after the fact. The agent also [runs builds and tests to validate generated code](https://aws.amazon.com/about-aws/whats-new/2025/01/amazon-q-developer-agent-builds-tests-validate-generated-code-real-time/) in real time, only exiting its loop when it determines the candidate patches are ready.

**Cognition's Devin** makes the distinction between evaluation methods explicit. Their [evaluation framework](https://cognition.ai/blog/evaluating-coding-agents) prioritizes classical verification — compilers, linters, type checkers, unit tests — because these are deterministic and cheap, mirroring Patterns 1-2 (trajectory and dataset evals) in this demo. For outcomes too complex for classical checks, Cognition deploys evaluator agents that have access to Devin's browsing, shell, and code editing tools to autonomously judge results — the LLM-as-judge pattern (Pattern 3) applied to the agent's own output. Devin also [self-reviews before creating PRs](https://cognition.ai/blog/devin-review), catching roughly 30% more issues than PRs submitted without the review step. This two-tier approach — fast deterministic checks first, expensive LLM judges second — mirrors the 3-Phase Eval Strategy described earlier in this post, except the harness runs both tiers automatically on every task.

The pattern across all four harnesses is consistent: evaluation is not a separate activity that happens after development — it is woven into the agent loop. The agent edits, evaluates, and iterates before the human ever enters the picture. The eval patterns in this demo give you the building blocks; these harnesses show what it looks like when you make them load-bearing parts of the agent itself.
