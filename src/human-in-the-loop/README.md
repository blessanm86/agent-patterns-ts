...
Human-in-the-loop is arguably the most visible pattern in coding agent harnesses -- every one of them has to decide how much autonomy the agent gets before asking the developer. The result is a spectrum that runs from "approve every file read" to "let it rip," and the most interesting engineering happens in between.

**Claude Code** implements the most layered permission system of any harness. It evaluates every action through a six-tier pipeline: hooks (custom pre/post logic), deny rules, allow rules, ask rules, the current mode, and finally a UI callback. On top of this, an OS-level sandbox built on macOS [seatbelt](https://www.anthropic.com/engineering/claude-code-sandboxing) and Linux bubblewrap restricts filesystem and network access at the kernel level -- even if a prompt injection convinces the model to run `rm -rf /`, the sandbox blocks it. A smaller model (Haiku) classifies bash commands as safe or unsafe before they reach the user, reducing noise from harmless operations. The combined effect is striking: Anthropic reports that [sandboxing safely reduces permission prompts by 84%](https://www.anthropic.com/engineering/claude-code-sandboxing) while actually _improving_ security outcomes, because users pay attention to the prompts that remain. The philosophy is that the primary safety layer should be the sandbox (bounded autonomy), not per-action approval -- HITL becomes a trust and UX mechanism rather than the last line of defense.

**Cline** takes a fundamentally different approach: the agent literally suspends mid-execution and waits for the human. Internally, Cline's agent loop calls an `ask()` method that returns a Promise. The loop `await`s that Promise, and nothing happens until the VS Code webview posts a response back -- the entire Node.js event loop is free, but the agent is frozen at that exact decision point. When the user clicks approve, deny, or edits the proposed action, the Promise resolves with the response and execution resumes from the same line. This is elegant because it requires zero serialization of agent state; the JavaScript call stack _is_ the checkpoint. The downside is durability -- if VS Code crashes, the suspended Promise is lost. On the opposite end of Cline's spectrum sits [YOLO mode](https://docs.cline.bot/features/yolo-mode), which auto-approves everything: file changes, terminal commands, browser actions, MCP tools. Between these extremes, Cline offers granular [auto-approve categories](https://docs.cline.bot/features/auto-approve) (read files, edit files, safe commands, browser, MCP) where each category can be toggled independently. Rather than a static allowlist, the model itself marks each command with a `requires_approval` flag based on the command and its arguments -- build commands and read-only queries pass through, while `rm`, `sed -i`, and package installs trigger the approval gate.

**OpenAI Codex CLI** simplifies the dial to [three modes](https://developers.openai.com/codex/cli/features/): read-only (consultative -- browse files but approve all changes), auto (read/edit/run within the working directory, ask for anything outside), and full access (unrestricted, including network). The cloud-hosted Codex takes isolation further with a [two-phase container model](https://developers.openai.com/codex/security): the setup phase runs with network access and can install dependencies and inject secrets, then the agent phase runs with secrets removed and network disabled by default. This separation means the agent can never exfiltrate credentials it used during setup -- the attack surface is architecturally eliminated rather than policy-gated. Users can adjust permissions mid-session via `/permissions`, and regardless of mode, Codex surfaces a full transcript of actions so developers can review and roll back through normal git workflows.

**Roo Code** introduces the most structured per-mode restrictions. Each [custom mode](https://docs.roocode.com/features/custom-modes) declares exactly which tools it can access, and the Orchestrator mode intentionally has _no_ file-editing tools -- it can only delegate to specialized modes that do. File-level permissions use regex patterns matched against workspace-relative paths (e.g., `src/components/.*\.tsx`), creating fine-grained boundaries. The [auto-approve system](https://docs.roocode.com/features/auto-approving-actions) recognizes eight permission categories with longest-prefix matching for command allow/deny lists, and it blocks dangerous parameter substitutions even in otherwise-permitted commands. The separation between "what tools a mode can see" and "what the user auto-approves" creates two independent permission layers -- one controlled by the harness designer, the other by the end user.

**Windsurf** and **Devin** represent the enterprise end of the spectrum. Windsurf classifies every terminal command as safe or unsafe, with [administrators able to set organization-wide maximum auto-execution levels](https://docs.windsurf.com/windsurf/terminal) and enforce team-wide command allowlists and denylists -- individual users can restrict further but never exceed the admin ceiling. [Devin](https://docs.devin.ai/enterprise/deployment/overview) takes a different approach entirely: its stateless "Brain" (the LLM reasoning layer) is architecturally separated from the stateful "Devbox" (the VM where code runs), and enterprise customers can deploy the Devbox inside their own VPC so that code and secrets never leave their network. Each session runs in an isolated VM, and the Brain communicates with it through a controlled interface rather than having direct filesystem access. This is HITL at the infrastructure level -- rather than asking "may I run this command?", the system is designed so that the command _can only affect_ a sandboxed environment.

The pattern across all these harnesses validates a key insight from this demo: the most effective permission systems are not binary approve/deny gates but _layered_ defenses where sandboxing handles the security floor, risk classification reduces prompt noise, and human approval is reserved for the genuinely ambiguous decisions that only a human can make.
