...
The dual return pattern appears throughout production coding agents, though it manifests differently depending on the harness architecture. Every major harness faces the same core tension this pattern addresses: the user needs rich, visual feedback while the model needs concise, structured context.

**Claude Code** implements dual return at two levels. First, at the tool level: when a tool produces output larger than 50K characters, Claude Code [persists the full result to disk](https://github.com/anthropics/claude-code/issues/175) and injects only a truncated preview into the model's context window. The user can access the complete output via a file reference, but the model reasons over the summary. Second, at the agent level: when the [Task tool spawns a sub-agent](https://code.claude.com/docs/en/sub-agents), the sub-agent runs in an isolated context window and produces a potentially lengthy transcript of tool calls, file reads, and intermediate reasoning. Only a concise summary returns to the main agent's context. The full transcript stays quarantined. This is dual return applied to entire agent conversations -- the main agent gets the "content" (a summary), while the system retains the "artifact" (the full sub-agent transcript) for debugging and session replay.

**OpenAI's Codex** bakes dual return directly into its [App Server protocol](https://developers.openai.com/codex/app-server/). Every operation produces an Item with an explicit lifecycle: `item/started`, streaming `item/*/delta` events, and `item/completed`. A `commandExecution` item, for example, streams `outputDelta` events to the client for real-time terminal rendering, but the model receives the final `aggregatedOutput` in the completed item. File changes follow the same split: a `fileChange` item streams the underlying `apply_patch` tool call response for client display, while a separate `turn/diff/updated` notification emits an aggregated unified diff across all files in the turn. The protocol makes the two channels -- client display stream and model conversation state -- first-class architectural primitives rather than an afterthought.

**Cursor** achieves dual return through its [two-model architecture](https://cursor.com/blog/instant-apply). A primary LLM generates a "sketch" of the intended change -- focusing on core logic and intent rather than precise syntax. A separate, custom-trained Apply model then integrates the sketch into the actual codebase. The user sees inline diffs in the editor (additions in green, deletions in red) and can accept or reject each change. The model that produced the edit never saw or produced those diffs -- it worked with a higher-level sketch representation. The visual diff _is_ the user-facing artifact; the sketch is the model-facing content. This separation is what enables Cursor's speculative edits to reach approximately 1000 tokens per second on 70B parameter models.

**Aider** takes yet another approach: the model produces [SEARCH/REPLACE blocks](https://aider.chat/docs/more/edit-formats.html) -- structured text with exact original lines and their replacements -- while the user sees git-style unified diffs with `+`/`-` markers and context lines. The model's output format is optimized for reliable parsing and application (no line numbers, clear delimiters, literal code matching). The user's display format is optimized for human review (familiar diff syntax, color coding, surrounding context). Aider maintains a zoo of seven or more edit formats tuned per model family, each one a different answer to the same dual return question: what representation works best for the model vs. what representation works best for the human?
