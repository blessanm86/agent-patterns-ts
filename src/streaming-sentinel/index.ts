...
import { runSentinelAgent, runSeparateCallAgent } from "./agent.js";
...
async function handleChat(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  let body: { message?: string; history?: Message[]; mode?: string };
  try {
    body = JSON.parse(raw);
  } catch {
  ...
  const mode = body.mode === "separate" ? "separate" : "sentinel";
  ...
  try {
    const agent = mode === "sentinel" ? runSentinelAgent : runSeparateCallAgent;
    const updatedHistory = await agent(userMessage, history, emit);

    // Send conversation history so client can maintain state
...
server.listen(PORT, () => {
  console.log(`\n  Streaming Sentinel Agent Server`);
  console.log(`  ────────────────────────────────`);
