...
The coding agent harnesses shipping today reveal dramatically different philosophies about tool design — and the disagreements map directly to the techniques in this post.

**Claude Code takes the minimalist path.** Its core tool surface is built around a handful of primitives — Read, Edit, Write, Bash, Grep, Glob — each doing one thing well. The [tool descriptions](https://gist.github.com/wong2/e0f34aac66caf890a332f7b6f9e2ba8f) are textbook examples of every technique from this demo: verb-first descriptions, explicit "when to use" and "when NOT to use" clauses, inline format examples, and parameter names that encode constraints (`file_path` must be absolute, `old_string` must be unique in the file). The Edit tool enforces a Read-before-Edit requirement — you literally cannot edit a file you haven't read first — which is the poka-yoke principle in action. Bash acts as a universal adapter: anything not covered by a dedicated tool (running tests, installing packages, calling APIs) goes through the shell. The philosophy, as Anthropic's engineering team [has described it](https://www.anthropic.com/engineering/claude-code-best-practices), is to put "as little code as possible around the model" — minimal tools with rich descriptions, letting the model compose complex workflows from simple pieces, much like piping Unix commands together.

**Cursor takes the opposite approach: more tools, each purpose-built for a specific IDE operation.** Its [system prompt](https://gist.github.com/sshh12/25ad2e40529b269a88b80e7cf1c38084) exposes around 13 tools covering file editing, codebase search, terminal commands, browser actions, and task management. Where Claude Code gives the model `Bash` and trusts it to figure out `npm test`, Cursor provides a dedicated tool for each workflow step. The trade-off is the one this demo measures: more tools means more opportunities for the model to pick the wrong one, so each description needs careful disambiguation. Cursor compensates with a two-model architecture — a thinking model decides _what_ to do, and a specialized apply model handles _how_ — which offloads some of the tool-selection pressure from descriptions alone.

**Roo Code introduces a third idea: contextual tool availability.** Rather than giving the model the same tool set for every task, Roo Code's [mode system](https://docs.roocode.com/basic-usage/using-modes) restricts which tools are visible depending on the current mode. In Code mode, the agent gets full access to file editing and command execution. In Ask mode, it can only read — no writes, no shell. Most strikingly, the Orchestrator mode (the "[Boomerang](https://docs.roocode.com/features/boomerang-tasks)" pattern) has _no file tools at all_ — it can only delegate to sub-agents and read their completion summaries. This is tool description engineering taken to its logical extreme: the strongest "when NOT to use" clause is removing the tool entirely. Roo Code's designers found that giving the orchestrator file-read access caused it to fill its context with irrelevant file contents instead of staying focused on high-level task decomposition.

**Manus abandons structured tool calls altogether.** Its [CodeAct approach](https://gist.github.com/renschni/4fbc70b31bad8dd57f3370239dccd58f) has the model generate executable Python code instead of invoking named tools with JSON parameters. Need to search a codebase? Write a Python script that calls `grep`. Need to edit a file? Write code that reads, modifies, and writes it back. The agent's "tools" are just code libraries available in the sandbox. This eliminates the tool description problem entirely — the model doesn't need to parse a description to decide which function to call, because it's writing the implementation directly. The trade-off is that code generation is more token-expensive and harder to constrain than a discrete tool call, which is why Manus uses [logit masking](https://manus.im/blog/Context-Engineering-for-AI-Agents-Lessons-from-Building-Manus) to control which actions are available at any given point in the loop.

**Cline and Aider show how tool _formatting_ is itself an engineering surface.** Cline uses [XML-style tool tags](https://github.com/cline/cline) with explicit parameter delimiters, and adapts its formatting to the model being used — different models get different delimiters and structures for the same logical tool. Aider maintains a [zoo of 7+ edit formats](https://aider.chat/docs/more/edit-formats.html) (search/replace blocks, unified diffs, whole-file rewrites) and selects the format based on which model family is running, because the same tool _description_ produces different accuracy depending on whether the model was trained on diff syntax or search/replace syntax. These harnesses treat the tool description format as a hyperparameter to tune per model, not a fixed API contract.

The spectrum from Claude Code's 4 core primitives to Cursor's 13 specialized tools to Manus's "no tools, just code" is not a maturity curve — each represents a deliberate engineering trade-off. Fewer tools means simpler descriptions but more model reasoning. More tools means richer guardrails but higher selection error rates. No tools means maximum flexibility but no structured constraints. The techniques in this demo — parameter naming, when-not-to-use clauses, inline examples, actionable errors — apply regardless of where on that spectrum you land.
