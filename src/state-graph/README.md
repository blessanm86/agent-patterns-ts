...
Every coding agent harness is a state machine. The question is how explicitly it models its states. The spectrum runs from a bare `while(true)` with a single break condition all the way to named phases with enforced transitions — and the interesting insight is that all of them land somewhere on this continuum, not at one extreme.

**Claude Code** sits at the implicit end. Its [TAOR loop](https://vrungta.substack.com/p/claude-code-architecture-reverse) (Think-Act-Observe-Repeat) is roughly 50 lines of orchestration code: a `while(true)` that calls the model, checks for tool calls, executes them, and loops again. The states — "thinking," "acting," "observing" — exist, but they live in the developer's mental model rather than in named variables or an enum. The design philosophy is explicit: "the runtime is dumb; the model is the CEO." This is the same implicit state graph we opened this post with — three nodes, three edges, all unnamed. It works because the model itself handles routing, and the harness trusts it to decide when to stop.

**OpenAI Codex** takes a step toward explicitness. Its [agent loop](https://openai.com/index/unrolling-the-codex-agent-loop/) breaks down into five recognizable phases: prompt assembly (gathering instructions, tools, and user inputs into a JSON payload), LLM inference (streaming output events from the Responses API), tool execution (running requested tools and collecting results), inner loop iteration (appending tool results back into context for another inference pass), and completion (the model emits a "done" event with a response message). These phases aren't necessarily encoded as an enum or state variable in the code, but OpenAI's own engineering blog names them as distinct steps with clear transition rules. The inner loop is an explicit sub-cycle within the outer conversation turn — a state machine with a nested loop, rather than a single flat while loop.

**GitHub Copilot's** coding agent takes this further with a [structured multi-phase workflow](https://github.blog/ai-and-ml/github-copilot/github-copilot-coding-agent-101-getting-started-with-agentic-workflows-on-github/): assignment (an issue is delegated to `@copilot`), analysis (the agent examines the repository structure), planning (a structured task decomposition with progress tracking visible to the user), execution (code edits and terminal commands), verification (automatic detection of build errors, test failures, and syntax issues), iteration (course-correcting based on verification results), and completion (opening a pull request). Each phase has defined entry and exit conditions. The agent can't skip from assignment to execution — it must pass through analysis and planning first. This is the closest any mainstream harness comes to the explicit state graph pattern we built in this demo, with named phases enforcing a guaranteed ordering.

**Windsurf (Codeium)** introduces a twist: rather than a single state machine, [Cascade](https://docs.windsurf.com/windsurf/cascade/cascade) runs two concurrent agents with state-based coordination between them. A specialized planning agent continuously refines the long-term strategy while the primary model executes short-term actions based on that plan. They synchronize through shared todo lists and bidirectional updates — the execution agent can trigger plan modifications when it discovers new information. This is a state graph with two parallel tracks that share state, closer to a multi-agent workflow graph than a single-threaded loop.

**Cline** demonstrates yet another pattern: a recursive agent loop with [approval-gate suspension](https://github.com/cline/cline). Its core `recursivelyMakeApiRequests()` function runs the model, parses the response, and when an action requires approval (a file edit, a terminal command), it suspends via a Promise and waits for the user. Once approved, the Promise resolves and the loop resumes from exactly where it paused. This is a state machine with an explicit "suspended-waiting-for-human" state that the others don't have — the harness can be frozen mid-execution and resumed later. It's the same concept as the human-in-the-loop interrupt in LangGraph, but implemented through JavaScript's native async/await rather than a graph framework's checkpoint system.

The pattern across all five harnesses reinforces this post's central claim: **the graph is already there**. Claude Code's unnamed while loop, Codex's five-phase breakdown, Copilot's multi-step workflow, Windsurf's dual-agent coordination, and Cline's suspendable recursion are all state machines with nodes, edges, and conditional routing. They differ only in how visible they make the structure — and that visibility is a design choice driven by each harness's needs, not an architectural accident.
