...
Every major coding agent harness implements some form of self-validation, but they sit at different points along a spectrum: **pre-validation** (check before applying), **inline validation** (check immediately after each edit), **post-validation** (review the entire artifact before delivery), and **structural validation** (a dedicated critic agent that can trigger rollbacks). The differences reveal real design tradeoffs between latency, thoroughness, and architecture complexity.

**Cursor** occupies the pre-validation end of the spectrum with its [shadow workspace](https://cursor.com/blog/shadow-workspace) -- a hidden Electron window running a full LSP instance, completely isolated from the user's editor. When the AI proposes an edit, the change is applied inside the shadow window first, and the LSP reports lints back before anything touches the real workspace. As the Cursor team put it, "letting AIs get lints for their edits is one of the most impactful ways to improve code generation performance." The architectural cost is significant -- a separate window with its own language server instances, IPC via gRPC between extension hosts, and request interleaving to share resources -- but the payoff is that type errors and syntax problems never reach the user's files. This is the purest form of the "validate before delivering" principle from our demo, except the validator is the full language toolchain rather than a Zod schema.

**Aider** and **OpenCode** represent the inline validation approach -- check immediately after every edit and auto-fix in a tight loop. Aider runs [auto-lint and auto-test](https://aider.chat/docs/usage/lint-test.html) after each code change by default. When the linter or test suite returns a non-zero exit code, the error output is fed directly back to the LLM, which attempts a fix -- the same generate-validate-fix cycle as our demo's three-layer validation. OpenCode takes a similar approach but uses [LSP diagnostics](https://opencode.ai/docs/lsp/) instead of a standalone linter: after every file write, it queries the language server, collects diagnostics from the target file and up to five affected files, and appends them to the tool output so the model sees the errors inline. OpenCode's [9-level progressive fuzzy matching](https://cefboud.com/posts/coding-agents-internals-opencode-deepdive/) -- from exact match through whitespace-normalized to Levenshtein distance -- is itself a form of structural self-validation on the edit application step, gracefully degrading rather than failing outright when the model's output drifts from the original text.

**Devin** validates at the other end of the pipeline -- after the entire task is done but before the PR reaches a human. Devin 2.2 introduced [self-review](https://cognition.ai/blog/introducing-devin-2-2): the agent plans, codes, reviews its own output, catches issues, and fixes them before you ever open the PR. Cognition reports this catches roughly [30% more issues](https://cognition.ai/blog/devin-review) compared to PRs submitted without the review step -- unused variables, inconsistent error handling, missing null checks, test coverage gaps. This is post-validation: cheaper than per-edit checking, but it operates on the full diff rather than individual edits, so it catches cross-cutting issues that inline validators miss. **Claude Code** takes a prevention-first approach rather than post-hoc validation: the Read-before-Edit requirement ensures the model always sees the current file state before modifying it, and the [hooks system](https://code.claude.com/docs/en/hooks) lets users wire in custom validators (linters, formatters, test suites) that run automatically on `PostToolUse` events after file writes.

**Amazon Q Developer** pushes furthest into structural validation with a dedicated [3-agent debugger subsystem](https://aws.amazon.com/blogs/devops/dissecting-the-performance-gains-in-amazon-q-developer-agent-for-code-transformation/). A Memory agent tracks relevant findings across iterations, a Critic agent analyzes progress and detects dead-end solution paths, and a Debugger agent uses the memory and critique to modify its plan. When the Critic determines the current approach is unproductive, it triggers a rollback to a previous state -- true backtracking, not just retry. This is the most sophisticated validation architecture in any harness surveyed: rather than checking individual outputs against a schema, an entire agent is dedicated to evaluating whether the overall strategy is working. It maps directly to the Evaluator-Optimizer pattern from Anthropic's research, scaled up from a single tool call to a multi-iteration planning loop.

The lesson across these harnesses reinforces what our demo shows at a smaller scale: deterministic external feedback beats self-assessment every time. Whether it is Zod schema errors, LSP diagnostics, test suite output, or a critic agent's verdict, the pattern is the same -- give the model structured, actionable feedback from a source outside itself, and it can fix what it broke.
