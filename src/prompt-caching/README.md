...
Prompt caching is not just a cost optimization toggle — in production coding agents, it has become an architectural force that shapes how entire systems are designed. The most dramatic example is [Manus](https://manus.im/blog/Context-Engineering-for-AI-Agents-Lessons-from-Building-Manus), which treats KV-cache hit rate as "the single most important metric for a production-stage AI agent." With Claude Sonnet, cached input tokens cost $0.30/MTok while uncached tokens cost $3.00/MTok — a 10x difference. Given that agentic workloads exhibit roughly a 100:1 input-to-output token ratio (the agent re-reads its entire context on every turn but generates only a short action), even small drops in cache hit rate translate to massive cost spikes. This economic reality drove Manus to redesign its entire architecture around cache preservation.

Manus's context is strictly **append-only** — the system never edits or removes prior messages, tool results, or observations. This ensures the existing prefix remains byte-identical on every turn, maximizing cache reuse. But append-only creates a problem: how do you change what tools the agent can use at different stages without modifying the tool definitions that sit in the cached prefix? Manus solves this with **logit masking** — all tools remain in the prompt at all times, but a context-aware state machine constrains which tool tokens the model is allowed to emit during decoding. Tools use consistent prefixes (`browser_`, `shell_`) so constraints can be applied efficiently. The prompt never changes; only the decoder's output space changes. When the context window fills up, Manus treats the **filesystem as extended memory**, writing summaries and intermediate results to disk and dropping the content from context while preserving file paths — a "restorable compression" that keeps the prefix intact while freeing tokens.

**Claude Code** takes a different but equally cache-conscious approach. Its system prompt (~4,000 tokens) is designed as a stable prefix that always clears Anthropic's minimum caching threshold. Dynamic per-turn context — git status, open file contents, CLAUDE.md project instructions — is injected via `<system-reminder>` tags in the message stream rather than in the system prompt itself, so the system prompt prefix stays byte-identical across turns. The cache hierarchy follows the order tools, then system prompt, then messages, with [cache breakpoints](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching) placed strategically so that each layer can be cached independently. The result: an active coding session with messages every minute or two keeps the cache warm indefinitely, and Anthropic reports [up to 90% cost reduction](https://www.anthropic.com/news/prompt-caching) on cached input tokens. When Claude Code compacts a long conversation, it preserves the system prompt and tool definitions verbatim and appends the compaction summary as a new user message, ensuring the expensive prefix cache survives the compaction boundary.

**OpenAI Codex** demonstrates how cache awareness extends to infrastructure design. Its cloud environment uses a [two-phase container model](https://developers.openai.com/codex/cloud/environments/): a setup phase installs dependencies and configures the workspace, then an agent phase runs the coding loop. This separation means the setup-derived prompt prefix (instructions, tools, initial context) remains stable across all agent turns, maximizing OpenAI's automatic prefix caching. A [notable bug](https://openai.com/index/unrolling-the-codex-agent-loop/) in early MCP support revealed how fragile cache efficiency can be: MCP servers enumerated tools in non-deterministic order across requests, causing the tool definitions portion of the prompt to differ between turns and silently breaking cache hits. The fix was enforcing stable tool ordering — a single-line change with outsized cost impact. Codex also appends configuration changes as new messages rather than editing earlier ones, preserving the prefix match that caching depends on.

**Cursor** rounds out the picture from the IDE side. When Cursor compacts a long conversation to reclaim context space, it preserves signals that maintain prefix stability — retaining the system prompt and tool definitions unchanged so the cached KV vectors survive the compaction. This is a subtler form of cache-aware design: the compaction algorithm itself is constrained by caching economics. A [recent study on prompt caching in agentic tasks](https://arxiv.org/html/2601.06007v1) confirms the pattern these harnesses discovered empirically: caching only the stable prefix (system prompt and tool definitions) while leaving dynamic content uncached provides the most consistent cost and latency benefits, and naive full-context caching can paradoxically increase latency by triggering expensive cache writes that never get reused.
