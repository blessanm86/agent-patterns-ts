...
export const tools: ToolDefinition[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Create a hotel reservation once the guest has confirmed all details. Can include optional guest contact info.",
      parameters: {
        type: "object",
        properties: {
          guest_name: {
  ...
  },
  {
    ...
    function: {
      name: "get_guest_info",
      description:
        "Look up a returning guest's contact details by name. Returns their email and phone on file.",
      parameters: {
        type: "object",
        properties: {
          guest_name: {
            type: "string",
            description: "Full name of the guest to look up",
          },
        },
        required: ["guest_name"],
      },
...
// Mock guest database — PII that the redaction middleware should catch
const MOCK_GUESTS: Record<string, { email: string; phone: string }> = {
  "john smith": { email: "john.smith@example.com", phone: "555-123-4567" },
  "alice johnson": { email: "alice.j@corporate.net", phone: "555.987.6543" },
  "bob williams": { email: "bob.w@gmail.com", phone: "555-222-3333" },
};
...
function createReservation(args: {
  ...
  const reservation: Reservation = {
    ...
    totalPrice,
    nights,
    ...(args.guest_email && { guestEmail: args.guest_email }),
    ...(args.guest_phone && { guestPhone: args.guest_phone }),
  };

  return JSON.stringify({ success: true, reservation });
}

function getGuestInfo(args: { guest_name: string }): string {
  const guest = MOCK_GUESTS[args.guest_name.toLowerCase()];
  if (!guest) {
    return JSON.stringify({
      found: false,
      message: `No guest record found for "${args.guest_name}"`,
    });
  }
...
export function executeTool(name: string, args: Record<string, string>): string {
  switch (name) {
    ...
    case "get_guest_info":
      return getGuestInfo(args as Parameters<typeof getGuestInfo>[0]);
    default:
